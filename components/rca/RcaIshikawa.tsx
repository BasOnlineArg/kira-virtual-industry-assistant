...
// ─── Canvas constants (faithful to original HTML) ─────────────────────────────
...
export default function RcaIshikawa({
  ...
  // Mutable render state — never causes re-renders
  ...
  useEffect(() => {
    ...
    // Assign with explicit type so TypeScript keeps the non-null type in closures
    ...
    function drawThumbs(sx: number, sy: number, images: CanvasImage[]) {
      ...
      if (images.length > 4) {
        ...
        ctx.beginPath()
        ctx.roundRect(ox, sy + 4 * (TH + TG), TH, TH, 3)
        ctx.fill()
        ctx.fillStyle = '#fff'
    ...
    function drawBoard(sx: number, sy: number, cat: (typeof CATS)[number], idx: number) {
      ...
      if (d.text || d.causes.length) {
        ...
        ctx.font = '8px sans-serif'; ctx.fillStyle = '#bbb'
        ctx.textAlign = 'center'; ctx.textBaseline = 'middle'
    ...
    function drawAxisBox(
      ...
      if (summary) {
        ctx.font = '8px sans-serif'; ctx.fillStyle = 'rgba(255,255,255,.9)'
        summary.split('\n').slice(0, 3).forEach((l, i) =>
          ctx.fillText(trunc(l, bw - 14), sx, y + 18 + i * 11))
      } else {
        ctx.font = '8px sans-serif'; ctx.fillStyle = 'rgba(255,255,255,.45)'
    ...
    function draw() {
      ...
      ctx.strokeStyle = '#1a4faf'; ctx.lineWidth = Math.max(1, 3 * zoom); ctx.stroke()

      // Axis junction dots
      BX.forEach(bx => {
        const pt = p3(bx, 0, 0)
        ctx.beginPath(); ctx.arc(pt.x, pt.y, Math.max(2, 5 * zoom), 0, Math.PI * 2)
        ctx.fillStyle = '#1a4faf'; ctx.fill()
      })

      // Sort spines back-to-front for painter's algorithm
    ...
    function center() {
      ...
      CATS.forEach(cat => {
        const s = spine(cat)
        const tp = p3(s.tip.x, s.tip.y, s.tip.z)
        pts.push({ x: tp.x - PW / 2, y: tp.y })
        pts.push({ x: tp.x + PW / 2, y: tp.y + PH })
      })
    ...
    function loop() {
      ...
      if (s.easing) {
        s.panX = lerp(s.panX, s.tPX, 0.12)
        s.panY = lerp(s.panY, s.tPY, 0.12)
    ...
    function onDblClick(e: MouseEvent) {
      ...
      for (const h of rs.current.hitAxis) {
        if (mx >= h.x0 && mx <= h.x1 && my >= h.y0 && my <= h.y1) {
          if (h.key === 'inspector') {
            setInspText(inspRef.current.text)
            setInspImages([...inspRef.current.images])
            setInspOpen(true)
          } else {
            setProbW2H({ ...probRef.current.w2h })
            setProbImages([...probRef.current.images])
            setProbOpen(true)
          }
          return
        }
      ...
      for (const h of rs.current.hitBoards) {
        if (mx >= h.x0 && mx <= h.x1 && my >= h.y0 && my <= h.y1) {
          ...
          setCatObs(d.text)
          setCatCauses([...d.causes])
          setCatImages([...d.images])
          setCauseInp('')
  ...
  // ── Modal close handlers (save to shared state) ───────────────────────────
  ...
  // ─── Render ─────────────────────────────────────────────────────────────────
  return (
    <div style={{ background: '#eef0f3', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, padding: '10px 0' }}>
      {/* Controls hint */}
      <div style={{ fontSize: 11, color: '#555', display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'center' }}>
      ...
      <canvas
        ...
        height={CH}
        tabIndex={0}
        style={{ borderRadius: 10, display: 'block', outline: 'none', cursor: 'grab', maxWidth: '100%' }}
      />
      ...
      {activeCatIdx !== null && activeCat && (
        <Overlay>
          <ModalBox>
            <div style={{ ...mboxHeader, background: activeCat.c }}>
              <h3 style={mboxTitle}>{activeCat.l}</h3>
              <button style={xbtn} onClick={closeCat}>✕</button>
            </div>
            <div style={mb}>
              {/* Guiding questions */}
              <div style={{ background: '#f8f9fa', borderRadius: 7, padding: '10px 13px' }}>
                <p style={{ fontSize: 10, fontWeight: 700, color: '#888', textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: 5 }}>
                  Preguntas guía
                </p>
                <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 3 }}>
                  {(QUESTIONS[activeCatIdx] ?? []).map((q, i) => (
                    <li key={i} style={{ fontSize: 11, color: '#444', paddingLeft: 12, position: 'relative', lineHeight: 1.4 }}>
                      <span style={{ position: 'absolute', left: 0, color: '#888', fontWeight: 700, fontSize: 9 }}>?</span>
                      {q}
                    </li>
                  ))}
                </ul>
              ...
              <textarea
                ...
                placeholder="Describí lo que observás en esta rama..."
                style={mta}
              />
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                {catCauses.map((c, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#f8f9fa', borderRadius: 6, padding: '5px 9px' }}>
                    <span style={{ flex: 1, fontSize: 12, color: '#333' }}>● {c}</span>
                    <button
                      onClick={() => setCatCauses(cs => cs.filter((_, j) => j !== i))}
                      style={{ background: 'none', border: 'none', color: '#bbb', cursor: 'pointer', fontSize: 12 }}
                    >✕</button>
                  </div>
                ))}
              </div>
              <div style={{ display: 'flex', gap: 6 }}>
                <input
                  type="text"
                  value={causeInp}
                  onChange={e => setCauseInp(e.target.value)}
                  onKeyDown={e => e.key === 'Enter' && addCause()}
                  placeholder="Agregar causa..."
                  style={{ flex: 1, padding: '6px 9px', border: '1.5px solid #e2e8f0', borderRadius: 6, fontSize: 12, fontFamily: 'inherit' }}
                />
                <button onClick={addCause} style={{ padding: '6px 12px', borderRadius: 6, border: 'none', background: '#4a90d9', color: '#fff', cursor: 'pointer', fontSize: 12 }}>
                  + Agregar
                </button>
              </div>
              ...
              {/* Images */}
              <div>
                <p style={{ fontSize: 11, fontWeight: 600, color: '#555', marginBottom: 6 }}>📎 Imágenes</p>
                <ImgGrid images={catImages} onRemove={i => setCatImages(imgs => imgs.filter((_, j) => j !== i))} />
                <label style={ubtn}>
                  📷 Adjuntar
                  <input type="file" accept="image/*" multiple style={{ display: 'none' }} onChange={handleCatFiles} />
                </label>
              </div>
            </div>
            <div style={mf}>
              <button onClick={closeCat} style={{ background: '#1a4faf', color: '#fff', border: 'none', borderRadius: 7, padding: '7px 18px', fontSize: 13, cursor: 'pointer' }}>
                Guardar y cerrar
              </button>
            </div>
          </ModalBox>
      ...
      {inspOpen && (
        <Overlay>
          <ModalBox>
            <div style={{ ...mboxHeader, background: '#b45309' }}>
              <h3 style={mboxTitle}>Insight Inspector</h3>
              <button style={xbtn} onClick={closeInsp}>✕</button>
            </div>
            <div style={mb}>
              ...
              <textarea
                ...
                placeholder="Observaciones, hipótesis o insights..."
                style={{ ...mta, minHeight: 120 }}
              />
              <div>
                <p style={{ fontSize: 11, fontWeight: 600, color: '#555', marginBottom: 6 }}>📎 Imágenes</p>
                ...
                <label style={ubtn}>
                  📷 Adjuntar
                  <input type="file" accept="image/*" multiple style={{ display: 'none' }} onChange={handleInspFiles} />
                </label>
              </div>
            </div>
            <div style={mf}>
              <button onClick={closeInsp} style={{ background: '#1a4faf', color: '#fff', border: 'none', borderRadius: 7, padding: '7px 18px', fontSize: 13, cursor: 'pointer' }}>
                Guardar y cerrar
              </button>
            </div>
          </ModalBox>
      ...
      {probOpen && (
        <Overlay>
          <ModalBox>
            <div style={{ ...mboxHeader, background: '#991b1b' }}>
              <h3 style={mboxTitle}>Problema — 5W2H</h3>
              <button style={xbtn} onClick={closeProb}>✕</button>
            </div>
            <div style={mb}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                {(
                  ...
                  <div key={k} style={full ? { gridColumn: '1/-1' } : {}}>
                    <label style={{ fontSize: 10, fontWeight: 700, color: '#888', textTransform: 'uppercase', letterSpacing: '.04em', display: 'block', marginBottom: 3 }}>
                      {label}
                    </label>
                    <textarea
                      rows={2}
                      ...
                      style={{ width: '100%', minHeight: 50, fontFamily: 'inherit', fontSize: 12, border: '1.5px solid #e2e8f0', borderRadius: 6, padding: 6, resize: 'vertical' }}
                    />
                  </div>
                ))}
              </div>
              <div>
                ...
                <label style={ubtn}>
                  📷 Adjuntar
                  <input type="file" accept="image/*" multiple style={{ display: 'none' }} onChange={handleProbFiles} />
                </label>
              </div>
            </div>
            <div style={mf}>
              <button onClick={closeProb} style={{ background: '#1a4faf', color: '#fff', border: 'none', borderRadius: 7, padding: '7px 18px', fontSize: 13, cursor: 'pointer' }}>
                Guardar y cerrar
              </button>
            </div>
          </ModalBox>
...
function Overlay({ children }: { children: React.ReactNode }) {
  return (
    <div style={{
      ...
      background: 'rgba(0,0,0,.55)', zIndex: 50,
...
function ModalBox({ children }: { children: React.ReactNode }) {
  return (
    <div style={{
      ...
      boxShadow: '0 12px 40px rgba(0,0,0,.22)', marginBottom: 30, overflow: 'hidden',
    }}>
      {children}
    </div>
  )
    ...
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, marginBottom: 7 }}>
      {images.map((img, i) => (
        <div key={i} style={{ position: 'relative', width: 66, height: 66 }}>
          ...
          <img src={img.src} alt="evidencia" style={{ width: 66, height: 66, borderRadius: 6, objectFit: 'cover', border: '2px solid #e2e8f0' }} />
          <button
            onClick={() => onRemove(i)}
            style={{ position: 'absolute', top: -5, right: -5, background: '#e53e3e', color: '#fff', border: 'none', borderRadius: '50%', width: 16, height: 16, cursor: 'pointer', fontSize: 9, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          >✕</button>
        </div>
      ))}
    </div>
...
const mb: React.CSSProperties = { padding: '14px 18px', display: 'flex', flexDirection: 'column', gap: 11 }
const slabel: React.CSSProperties = { fontSize: 12, fontWeight: 600, color: '#555' }
const mta: React.CSSProperties = {
  width: '100%', minHeight: 75, fontFamily: 'inherit', fontSize: 13,
  border: '1.5px solid #e2e8f0', borderRadius: 7, padding: 8, resize: 'vertical',
}
const mf: React.CSSProperties = {
  display: 'flex', justifyContent: 'flex-end',
  padding: '11px 18px', borderTop: '1px solid #f0f0f0',
}
const ubtn: React.CSSProperties = {
  display: 'inline-flex', alignItems: 'center', gap: 5,
  padding: '6px 12px', border: '1.5px dashed #cbd5e0', borderRadius: 7,
  fontSize: 11, color: '#718096', cursor: 'pointer', background: '#f8f9fa',
}
