...
export default function RcaAiPanel({
  ...
  // Auto-run AI on first mount if no result yet
  useEffect(() => {
    if (!aiData) runAI()
  ...
  function saveEvent() {
    ...
    if (editId) {
      setEvents(evs => evs.map(ev =>
        ev.id === editId ? { ...ev, ...evForm, type: evForm.type as TLEvent['type'] } : ev
      ))
    } else {
  ...
  function editEvent(ev: TLEvent) {
    setEditId(ev.id)
    setEvForm({ dt: ev.dt, type: ev.type, desc: ev.desc, resp: ev.resp })
    setShowForm(true)
  }
  ...
  function handleDrop(targetIdx: number) {
    ...
    setEvents(evs => {
      const arr = [...evs]
      const srcId  = sorted[dragSrc].id
      const dstId  = sorted[targetIdx].id
  ...
  const generatePDF = useCallback(() => {
    ...
    const imgsHtml = (images: { src: string }[]) =>
      images?.length
        ? `<div class="pdf-imgs">${images.map(img => `<img src="${img.src}" alt="evidencia"/>`).join('')}</div>`
        : ''
    ...
    const ishiSections = CATS.map((cat, i) => {
      ...
      return (
        ...
        `<div class="pdf-ishi-cat-head" style="background:${cat.c};">${cat.l}</div>` +
        `<div class="pdf-ishi-cat-body">` +
        (d.text ? `<div class="pdf-ishi-obs">${d.text}</div>` : '') +
        (d.causes.length ? `<ul class="pdf-ishi-causes">${d.causes.map(c => `<li style="color:${cat.c};">● ${c}</li>`).join('')}</ul>` : '') +
        imgsHtml(d.images) +
        `</div></div>`
      )
    }).filter(Boolean).join('')
    ...
    const tlHtml = sorted.length
      ? sorted.map(ev => {
          ...
          return (
            `<div class="pdf-tl-item pdf-no-break" style="background:${m.bg};border-left-color:${m.color};">` +
            `<div><div class="pdf-tl-type" style="color:${m.color};">${m.label}</div>` +
    ...
    const actionsHtml = ai?.acciones
      ? `<table class="pdf-actions-table"><thead><tr><th>#</th><th>Acción correctiva</th><th>Responsable</th><th>Plazo</th></tr></thead><tbody>` +
        ai.acciones.map((a, i) =>
          `<tr><td style="text-align:center;font-weight:700;">${i + 1}</td><td>${a.descripcion}</td><td>${a.responsable}</td><td>${a.plazo}</td></tr>`
        ).join('') + `</tbody></table>`
    ...
    pdfRef.current.innerHTML = `
      ...
      <div class="pdf-section pdf-page-break">
        ...
        <div class="pdf-body">El presente informe documenta la investigación de causa raíz iniciada a partir del siguiente evento: <strong>${w2h.what || '—'}</strong>, ocurrido el <strong>${w2h.when || '—'}</strong> en <strong>${w2h.where || '—'}</strong>.${w2h.howmuch ? ` El impacto estimado: ${w2h.howmuch}.` : ''}</div>
      </div>
      <div class="pdf-section">
        <div class="pdf-section-title">2. Metodología</div>
        <div class="pdf-method-box">El análisis fue realizado utilizando la metodología híbrida <strong>5W2H + Ishikawa (9 categorías) + Análisis por Inteligencia Artificial</strong>.</div>
      </div>
      <div class="pdf-section">
        <div class="pdf-section-title">3. Descripción del Evento — 5W2H</div>
        <table class="pdf-table"><thead><tr><th style="width:22%;">Pregunta</th><th>Respuesta</th></tr></thead>
        <tbody>
      ...
      <div class="pdf-section pdf-page-break">
        ...
        ${ai
          ? `<div class="pdf-section-sub">Causa raíz identificada</div>
             ...
             <p style="margin-bottom:4px;"><span class="pdf-risk pdf-risk-${ai.riesgo}">${ai.riesgo}</span></p>
             <p class="pdf-body">${ai.riesgo_justificacion || ''}</p>`
          : '<p style="color:#94a3b8;">Sin análisis generado</p>'
        }
      </div>
      <div class="pdf-section">
        <div class="pdf-section-title">7. Acciones Correctivas</div>
        ${actionsHtml}
      </div>
      <div class="pdf-section">
  ...
  const riskColor: Record<string, string> = {
    CRITICO: '#dc2626', ALTO: '#ea580c', MEDIO: '#d97706', BAJO: '#16a34a',
  }
  ...
  return (
    <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
      ...
      <div style={aiCard}>
        {/* Header */}
        <div style={{ background: '#0f172a', padding: '16px 22px', color: '#fff', display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontSize: 20 }}>🤖</span>
          <h2 style={{ fontSize: 16, fontWeight: 700, flex: 1, margin: 0 }}>Análisis de Causa Raíz — IA</h2>
          <button onClick={onBack} style={{ ...btnSm, marginLeft: 'auto' }}>← Ishikawa</button>
        </div>
        ...
        <div style={{ padding: '18px 22px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          {loading && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, padding: '40px 0' }}>
              <div style={{
                ...
                borderTopColor: '#1a4faf', borderRadius: '50%',
                animation: 'rca-spin 1s linear infinite',
              }} />
              <p style={{ fontSize: 14, color: '#64748b' }}>Analizando causas...</p>
            </div>
          )}

          {error && (
            <div style={{ background: '#fef2f2', borderRadius: 8, padding: 16, color: '#991b1b', fontSize: 13 }}>
          ...
          {aiData && !loading && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <ResultBlock
                headerBg={riskColor[aiData.riesgo] ?? '#64748b'}
                headerColor="#fff"
                title="🎯 Causa Raíz Identificada"
              >
                <p style={{ fontWeight: 600, fontSize: 14, margin: 0 }}>{aiData.causa_raiz}</p>
              </ResultBlock>
              ...
              <ResultBlock title="🔴 Riesgo Residual">
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <span style={{
                    display: 'inline-block', borderRadius: 20, padding: '3px 14px',
                    fontSize: 12, fontWeight: 700, color: '#fff',
                    background: riskColor[aiData.riesgo] ?? '#64748b',
                  }}>{aiData.riesgo}</span>
                  <span>{aiData.riesgo_justificacion}</span>
                </div>
              </ResultBlock>

              <ResultBlock title="✅ Acciones Correctivas">
                {aiData.acciones.map((a, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '7px 0', borderBottom: '1px solid #f0f4f8' }}>
                    <div style={{ width: 22, height: 22, borderRadius: '50%', background: '#1a4faf', color: '#fff', fontSize: 11, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      {i + 1}
                    </div>
                    <div>
                      <div>{a.descripcion}</div>
                      <div style={{ fontSize: 11, color: '#64748b', marginTop: 2 }}>👤 {a.responsable} · ⏱ {a.plazo}</div>
                    </div>
                  </div>
                ))}
              </ResultBlock>
              ...
              <ResultBlock title="📋 Conclusión Ejecutiva" headerBg="#0f172a" headerColor="#fff">
                <p style={{ fontStyle: 'italic', margin: 0 }}>{aiData.conclusion}</p>
              </ResultBlock>
            </div>
          )}
          ...
          {(aiData || error) && (
            <div style={{ borderRadius: 12, border: '1.5px solid #e2e8f0', overflow: 'hidden' }}>
              <div style={{ background: '#0f172a', padding: '12px 18px', display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <h3 style={{ color: '#fff', fontSize: 14, fontWeight: 700, flex: 1, margin: 0 }}>
              ...
              <div style={{ padding: 18 }}>
                {showForm && (
                  <div style={{ background: '#f8fafc', borderRadius: 10, padding: 14, display: 'flex', flexDirection: 'column', gap: 10, border: '1.5px solid #e2e8f0', marginBottom: 16 }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                      <div>
                        <label style={evLabel}>Fecha y hora</label>
                        <input type="datetime-local" value={evForm.dt}
                          onChange={e => setEvForm(f => ({ ...f, dt: e.target.value }))}
                          style={evInput} />
                      </div>
                      <div>
                    ...
                    <div>
                      ...
                      <input type="text" placeholder="Operador, turno..." value={evForm.resp}
                        onChange={e => setEvForm(f => ({ ...f, resp: e.target.value }))}
                        style={evInput} />
                    </div>
                ...
                {sorted.length === 0 ? (
                  ...
                  <div style={{ position: 'relative', paddingBottom: 24, paddingTop: 8 }}>
                    {/* Axis line */}
                    <div style={{ position: 'absolute', top: 36, left: 24, right: 24, height: 3, background: 'linear-gradient(90deg,#1a4faf,#7c3aed)', borderRadius: 2 }} />
                    {/* Nodes */}
                        ...
                        return (
                          ...
                          >
                            <div style={{ marginBottom: 6 }}>
                              <div style={{ width: 18, height: 18, borderRadius: '50%', border: '3px solid #fff', boxShadow: `0 0 0 2px ${m.color}`, background: m.color }} />
                            </div>
                            ...
                            <div style={{ fontSize: 10, color: '#94a3b8', textAlign: 'center', marginBottom: 4 }}>{fmtDt(ev.dt)}</div>
                            <div style={{ fontSize: 11, color: '#475569', textAlign: 'center', lineHeight: 1.3, maxWidth: 120 }}>{ev.desc || '—'}</div>
                            {ev.resp && (
                              <div style={{ fontSize: 10, color: '#94a3b8', textAlign: 'center', marginTop: 3 }}>👤 {ev.resp}</div>
                            )}
        ...
        {/* Footer */}
        {(aiData || error) && (
          <div style={{ padding: '14px 22px', borderTop: '1px solid #f0f0f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
...
function ResultBlock({
  title, headerBg = '#f1f5f9', headerColor = '#334155', children,
}: {
  ...
  return (
    <div style={{ borderRadius: 10, overflow: 'hidden' }}>
      ...
      <div style={{ padding: '11px 14px', fontSize: 13, lineHeight: 1.65, color: '#1e293b', background: '#f8fafc' }}>
        {children}
      </div>
    </div>
  )
...
const btnGhost: React.CSSProperties = {
  ...
  cursor: 'pointer', fontWeight: 600, background: '#fff', color: '#1a4faf',
  border: '1.5px solid #1a4faf',
}
...
const evInput: React.CSSProperties = {
  ...
  border: '1.5px solid #e2e8f0', borderRadius: 6, fontSize: 12, fontFamily: 'inherit',
}
